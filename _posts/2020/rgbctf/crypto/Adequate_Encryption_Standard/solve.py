...
def dec_perm(in_bytes: bytes) -> bytes:
    binary = bytearray(64)
    permuted = bin(int.from_bytes(in_bytes, 'big'))[2:].zfill(64)
    for i in range(64):
        binary[pbox[i]] = ord(permuted[i])
    num = int.to_bytes(int(binary,2),8,byteorder='big')
    return num
...
key_set = {expand_key(long_to_bytes(i),8):long_to_bytes(i) for i in range(65536)}
