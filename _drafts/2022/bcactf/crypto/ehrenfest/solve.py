import math
from Crypto.Util.Padding import pad
from Crypto.Util.number import bytes_to_long, long_to_bytes
from z3 import *

def all_smt(s, initial_terms):
    def block_term(s, m, t):
        s.add(t != m.eval(t))
    def fix_term(s, m, t):
        s.add(t == m.eval(t))
    def all_smt_rec(terms):
        if sat == s.check():
           m = s.model()
           yield m
           for i in range(len(terms)):
               s.push()
               block_term(s, m, terms[i])
               for j in range(i):
                   fix_term(s, m, terms[j])
               yield from all_smt_rec(terms[i:])
               s.pop()
    yield from all_smt_rec(list(initial_terms))


def chunks(l, n):
    assert(len(l) % n == 0)
    for i in range(0, len(l), n):
        yield l[i:i+n]

FLAG_LEN = 72
M_SIZE = math.isqrt(FLAG_LEN * 8)

def encrypt(ptxt,key):
    assert len(ptxt) == M_SIZE
    enc = [0] * M_SIZE
    for i in range(M_SIZE):
        for j in range(M_SIZE):
            if key[i][j] == 1:
                enc[i] ^= ptxt[j]
    return bytes(enc)

m = b'Reaction and diffusion of chemical species can produce a variety of patterns, reminiscent of those often seen in nature. The Gray Scott equations model such a reaction. For more information on this chemical system see the articles "Complex Patterns in a Simple System," by John E. Pearson and "Pattern Formation by Interacting Chemical Fronts," by K.J. Lee, W.D. McCormick, Qi Ouyang, and H.L. Swinney. These articles appeared in Science, Volume 261, 9 July 1993.'

messages = [i for i in chunks(pad(m, M_SIZE), M_SIZE)]

enc_m =  b''

encs = [i for i in chunks(bytes.fromhex(enc_m.decode()), M_SIZE)]

key = [[Bool(f'k_{i}_{j}') for i in range(M_SIZE)] for j in range(M_SIZE)]


def encrypt_constraint(pt, ct):
    enc = [BitVecVal(0,8) for _ in range(M_SIZE)]
    for i in range(M_SIZE):
        for j in range(M_SIZE):
            enc[i] = If(key[i][j], enc[i]^pt[j], enc[i])
    return [i==j for i,j in zip(enc, ct)]


def get_key():
    solver = Solver()
    for pt,ct in zip(messages, encs):
        solver.add(encrypt_constraint(pt,ct))

    keylist = []
    for row in key:
        keylist.extend(row)


    if solver.check() == sat:
        model = solver.model()
        key_flag = [[int(bool(model.eval(key[i][j]))) for j in range(M_SIZE) ] for i in range(M_SIZE)]
        flag_int = int("".join("".join(map(str,row)) for row in key_flag),2)
        print(long_to_bytes(flag_int))
        return key_flag

key_flag = get_key()

encoded_flag = bytes.fromhex("5846353541255b46652543007d776b6c3a4904233b0e2155445375275974105e64740d112d37687f3a48446825173b025e093a254c7a44513861044b6b236a242e13597f221824121d167c35142d5e586c36110925237e713f5500732d46624a7f40584a743f5c3a293b660d235571163167131e71063e41")

def charset_constraint(i):
    return Or(
            And(i>=ord('a'), i<=ord('z')),
            And(i>=ord('A'), i<=ord('Z')),
            And(i>=ord('0'), i<=ord('9')),
            i==ord('-'),
            i==ord('{'),
            i==ord('}'),
            i==5  # padding
            )


def decrypt(ct,key_flag):
    pt = [BitVec(f'pt_{i}',8) for i in range(M_SIZE)]
    enc = [0 for _ in range(M_SIZE)]
    for i in range(M_SIZE):
        for j in range(M_SIZE):
            if key_flag[i][j]==1:
                enc[i] ^= pt[j]
    solver = Solver()
    solver.add([i==j for i,j in zip(enc, ct)])
    # if solver.check()==sat:
    #     model = solver.model()
    #     model_dict = {str(i):model[i].as_long() for i in model.decls()}
    #     pt = bytes(model_dict.get(f'pt_{i}',0) for i in range(M_SIZE))
    #     valid_positions = [i for i in range(M_SIZE) if pt[i]]
    # solver = Solver()
    # solver.add([i==j for i,j in zip(enc, ct)])
    # for i in valid_positions:
    for i in range(M_SIZE):
        if i%8:
        # solver.add(And(i>0,i<127))
            solver.add(charset_constraint(pt[i]))
    for model in all_smt(solver, pt):
        model_dict = {str(i):model[i].as_long() for i in model.decls()}
        pt = bytes(model_dict.get(f'pt_{i}',0) for i in range(M_SIZE))
        print(pt)

flag_dec = b"bcactf{the-next-problem-requires-you-to-figure-out-what-the-encryption-key-i-used-was-so-have-fun-with-that-43b2_5}\x05\x05\x05\x05\x05"

for i,j in zip(chunks(flag_dec,M_SIZE), chunks(encoded_flag,M_SIZE)):
    assert(encrypt(i,key_flag)==j)
