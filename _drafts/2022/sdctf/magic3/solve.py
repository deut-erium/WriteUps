...
target = apply_perm_order(starting, [0,1])
def give_order(starting, target, num_steps):
    constraints = []

    selections = [Int(f'selection_{i}') for i in range(num_steps)]
    for s in selections:
        constraints.append(And(s>=0,s<6))
    ...
    def permute_on_selection(selection, inp, oup):
        return  If(selection==0, is_permutation(inp,oup, perms[0]),
                ...
                If(selection==4, is_permutation(inp,oup, perms[4]),
                                 is_permutation(inp,oup, perms[5]))))))
...
random_order = [random.randint(0,5) for _ in range(9)]
# target = apply_perm_order(starting, random_order)
result = solve(starting, target)
