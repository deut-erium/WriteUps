...
OUTPUT = list(map(bytes.fromhex,OUTPUT))

# FLAG = Array("FLAG", BitVecSort(8), BitVecSort(8))

def shuffle(s):
    bits = [Extract(i,i,byt) for byt in s for i in range(8)[::-1]]
    random.shuffle(bits)
    return [Concat(bits[i:i+8]) for i in range(0,len(bits),8)]

def swap(arr, i: int, j: int):
    arr = Store(arr, i, arr[i]^arr[j])
    arr = Store(arr, j, arr[j]^arr[i])
    arr = Store(arr, i, arr[i]^arr[j])
    return arr
    # arr[i] ^= arr[j]
    # arr[j] ^= arr[i]
...
def rc4_key_scheduling(key: bytes,keylen=37):
    ...
    j = 0
    for i in range(128):
        j = simplify((j + S[i] + key[i % keylen])&0x7f)
        S = swap(S, i, j)
    return S

def rc4_pseudo_random_generator(S):
    i = j = 0
    while True:
        i = (i + 1)&0x7f
        j = (j + S[i])&0x7f
        S = swap(S, i, j)
        yield S[(S[i] + S[j])&0x7f]
...
random.seed(2023)
FLAG = [BitVec(f'x_{i}',8) for i in range(len(OUTPUT[0]))]
FLAG_ORIG = FLAG[:]
solver = Solver()
prg = rc4_pseudo_random_generator(rc4_key_scheduling(FLAG))
for it in range(64):
    shuffled = shuffle(FLAG)
    key = [next(prg) for _ in range(len(FLAG))]
    for i,j,k in zip(key,shuffled,OUTPUT[it]):
        solver.add(i^j==k)

# for m in all_smt(solver,FLAG_ORIG):
#     ff = [m.eval(i).as_long() for i in FLAG_ORIG]
#     print(bytes(ff))
