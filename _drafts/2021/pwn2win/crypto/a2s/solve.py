from a2s import *
from z3 import BitVec, BitVecSort, BitVecVal, Function, Solver, If, simplify
...
while len(key_columns) < 3*4:
    ...
    if len(key_columns)%4==0:
        ...
        word[0]^=r_con[i]
        i+=1
    word = [ simplify(i^j) for i,j in zip(word,key_columns[-4])]
...
def inv_mix_columns_z3(s):
    for i in range(4):
        ...
        b = xtime_z3(xtime_z3(s[i][1] ^ s[i][3]))
        s[i][0] ^= u
        s[i][1] ^= v
        s[i][2] ^= u
        s[i][3] ^= v
...
for pt,ct in zip(plaintexts,ciphertexts):
    for i,j in zip(encrypt(pt),ct):
        solver.add(i==j)
    #solver.add(encrypt(pt)==ct)

#solver.add(KEY[0]==0x3)
#solver.add(KEY[-1]==0x39)
