...
class Curve(object):
    # Set attributes of a general Weierstrass cubic y^2 = x^3 + ax^2 + bx + c
    # over any field.
    ...
    def __str__(self):
        ...
        if self.char == 0:
            ...
            return self.eq + ' over ' + 'F_' + \
                str(self.char) + '^' + str(self.exp)

    # Compute the discriminant.
    def discriminant(self):
        a, b, c = self.a, self.b, self.c
        return -4 * a * a * a * c + a * a * b * b + \
            18 * a * b * c - 4 * b * b * b - 27 * c * c

    # Compute the order of a point on the curve.
    def order(self, P):
        Q = P
        orderP = 1
        # Add P to Q repeatedly until obtaining the identity (point at
        # infinity).
    ...
    def mult(self, P, k):
        if P.is_infinite():
            ...
            # Convert k to a bitstring and use peasant multiplication to
    ...
    # total when, after the ith doubling, the ith digit in the bitstring b is
    # a one.
    def repeat_additions(self, P, b, n):
        if b == '0':
            ...
            return self.add(P, self.repeat_additions(
                self.double(P), b[:-1], n + 1))
...
class CurveOverQ(Curve):
        ...
        for y in divisors(self.discriminant()):
            ...
            if const_term != 0:
                for x in divisors(const_term):
                    P = Point(x, y)
                    if 0 == x * x * x + self.a * x * x + self.b * \
                            x + const_term and self.has_finite_order(P):
                        points.append(P)
            # If the constant term is zero, factor out x and look for rational roots
            # of the resulting quadratic polynomial. Any such roots must divide
            # b.
            elif self.b != 0:
            ...
            # roots of the resulting linear polynomial. Any such roots must
            # divide a.
            elif self.a != 0:
    ...
    def add(self, P_1, P_2):
        ...
        # Use the slope of the tangent line or secant line to compute the
        # result.
        nu = P_1.y - ld * P_1.x
    ...
    def order(self, P):
        ...
        while not Q.is_infinite():
            Q = self.add(P, Q)
            orderP += 1
            # If we ever obtain non integer coordinates, the point has infinite
            # order.
    ...
    def torsion_group(self):
        ...
        # If this point generates the entire torsion group, the torsion group
        # is cyclic.
...
class CurveOverFp(Curve):
    ...
    def contains(self, P):
        if P.is_infinite():
            ...
            return (P.y * P.y) % self.char == (P.x * P.x * P.x + \
                    self.a * P.x * P.x + self.b * P.x + self.c) % self.char
    ...
    def get_points(self):
        ...
        for x in range(self.char):
            for y in range(self.char):
                P = Point(x, y)
                if (y * y) % self.char == (x * x * x + self.a * \
                    x * x + self.b * x + self.c) % self.char:
                    points.append(P)
        return points
    ...
    def add(self, P_1, P_2):
        ...
        if P_1.is_infinite():
            ...
            if P_1.y == 0:
                ...
                ld = ((3 * P_1.x * P_1.x + 2 * self.a * P_1.x + self.b)
                      * mult_inv(2 * P_1.y, self.char)) % self.char
        else:
...
def euclid(sml, big):
    ...
    if sml == 0:
        ...
        # gcd = y*(big - (big//sml)*sml) + x*sml = (x - (big//sml)*y)*sml +
        # y*big.
...
def mult_inv(a, n):
    ...
    # If gcd(a,n) = 1, and gcd(a,n) = x*a + y*n, x is the multiplicative
    # inverse of a.
    else:
        return x % n
...
def sign(message, curve, P, n, keypair):
    # Extract the private and public keys, and compute z by hashing the
    # message.
    d, Q = keypair
...
def crack_baby_giant(curve, P, n, Q):
    ...
    # stores b so that it can be quickly recovered after a matching giant step
    # hash.
    baby_table = {}
    for b in range(m):
        bP = curve.mult(P, b)
    ...
    # we have Q - gmP = bP, so extract b from the dictionary, then Q = (b +
    # gm)P.
    for g in range(m):
...
# Find d for which Q = dP using Pollard's rho algorithm. Assumes subgroup
# has prime order n.
...
def crack_rho(curve, P, n, Q, bits):
    ...
    # Compute a new random linear combination of P and Q to start the
    # cycle-finding.
    ...
    while True:
        ...
        # bits in the binary explansion of the x coordinate of the current
        # position.
        j = int(bin(T.x)[len(bin(T.x)) - bits: len(bin(T.x))], 2)
        T, aT, bT = curve.add(
            T, R_list[j][0]), (aT + R_list[j][1]) % n, (bT + R_list[j][2]) % n
        # Advance the hare two steps, again by adding points in R_list determined by the last
        # b bits in the binary explansion of the x coordinate of the current
        # position.
        for i in range(2):
            j = int(bin(H.x)[len(bin(H.x)) - bits: len(bin(H.x))], 2)
            H, aH, bH = curve.add(
                H, R_list[j][0]), (aH + R_list[j][1]) % n, (bH + R_list[j][2]) % n
        # If the tortoise and hare arrive at the same point, a cycle has been
        # found.
        if(T == H):
            break
    # It is possible that the tortoise and hare arrive at exactly the same
    # linear combination.
    if bH == bT:
        end_time = time()
...
# Find d from two messages signed with the same nonce k. Assumes subgroup
# has prime order n.
