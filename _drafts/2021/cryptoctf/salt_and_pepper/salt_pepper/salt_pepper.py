...
def auth_check(salt, pepper, username, password, h):
    return sha1(pepper + password + md5(salt +
                username).hexdigest().encode('utf-8')).hexdigest() == h
        ...
        if ans == 'l':
            ...
            try:
                inp_username, inp_password = [
                    bytes.fromhex(s) for s in inp.split(',')]
            ...
            if USERNAME in inp_username and PASSWORD in inp_password:
                if auth_check(
                        ...
                        inp_hash):
                    die(
                        f'| Congrats, you are master in hash killing, and it is the flag: {flag}')
                else:
