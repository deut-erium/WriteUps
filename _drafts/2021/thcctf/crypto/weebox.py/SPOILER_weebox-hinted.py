...
#
# About this:
#   - speaking of whiteboxes https://github.com/SideChannelMarvels (for AES)
#   - inspired by the WhibOx 2021 CHES CTF https://whibox.io/contests/2021/
#
# Hints:
#   - this file is an ECDSA whitebox :)
#   - reading the Wikipedia page gives you ideas for attacks:
#     https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm
#   - (side note, the implementation uses jacobian curve coordinates, but it's
#      unimportant! Try to attack the ECDSA whitebox... as a black box!)
#   - the tables are windowed multiplications:
#       https://en.wikipedia.org/wiki/Elliptic_curve_point_multiplication
#   - some noise has been added to the tables...
#       ...but where the « k » crypto-safe random is chosen here?
#   - do some faults!
#       -> by modifying the file and seeing what's happening...
#       -> ...or by bitflipping at random insides tables.bin :)
#   - good luck & have fun!
#
...
def invmod(a):
    return pow(a % moduli, moduli - 2, moduli) # (a * a⁻¹) % order == 1
...
gpt = (gptx, gpty, gptz) # (x, y) -> (x', y', z') where x == x' / z² & y == y' / z³
...
class scalar_table:
    ...
    def __call__(self, scalar):
        ...
        # scalar = w0, w1, w2, ... , w63        wi in [0 .. 15]
        #
        # windowing(table, scalar)  -> a * scalar
        #
        #   table
        #
        #   a * 2⁰                  a * b * 2⁰ + c
        ...
        #   a * 2¹⁵
        #
        #   table[w0] * 2⁰ + table[w1] * 2⁴ + table[w2] * 2⁸ + ..
        #
        # -> table of 16 integers * 64 words   -> 1024 integers
...
class point_table:
    def __init__(self, data, table):
        ...
        self.data = data
        #
        # -> table of 3 * 16 integers * 64 words   -> 1024 points
    ...
    def __call__(self, scalar):
        ...
        for idx in range(1, nb_words):
            ...
            #
            # ... 2**(4 * word_index) is already precomputed in the table point
            #                                    * taken into account
...
def verify(msg, sig, pub):
    ...
    r = int.from_bytes(r, byteorder='big') % moduli       # r, _ = k * G
    s = int.from_bytes(s, byteorder='big') % moduli       # s = k⁻¹ (e + r * priv)
    pub = load_pub(pub)

    # u1 & u2
    #

    s = invmod(s)           # s⁻¹           ->    k (e + r * priv)⁻¹
    u1 = mulmod(e, s)       # u1 = e * s⁻¹  ->    e k (e + r * priv)⁻¹
    u2 = mulmod(r, s)       # u2 = r * s⁻¹  ->    r k (e + r * priv)⁻¹
    ...
    # u1 * G          -> e * s⁻¹ * G
    # u2 * (priv * G) -> r * s⁻¹ * priv * G
    #
    # u1 * G + u2 * (priv * G) = G * (k * (e + r * priv)⁻¹ * (e + r * priv))
    #                          = G * (k * 1)
    #                          = G * k
    #                            \___/ -> x == r
...
def sign(msg):
    ...
    # r, _ = k * G
    #      = wololo(e, ie) * G          -> k is generated somehow here?
    #
    #      = sha2(e, priv) * G          (what's it's done sometimes in chips)

    t0 = la8b2f19f(e) % order           # t0 = a0 * e + b0      « + f(e) »¹ ?   ¹maybe some noise is added here?
    t1 = l193d94fd(t0) % order          # t1 = a1 * t0 + b1     « + f(t0) »²    ²maybe also here?
    t2 = mulmod(t1, lc987ed3a(ie))      # ...
    t3 = addmod(t2, l73dc1cd1(e))
    t4 = addmod(t3, la3f31e5b(ie))
    t5 = l0c7dced9(t4) # !! curve points !!

    tmp0 = l676247ea(e) % order

    t6 = scalarmult(t5, mulmod(invmod(tmp0), l23892dfe(e)))
    r, _ = to_affine(t6) # ---> k * G
    ...
    # private should be hidden somewhere here?
    #
    ...
    #
    # we're in python, it's easy to test stuff & make "perfect" faults?

    # we can also fault tables.bin
    #
